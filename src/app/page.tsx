...
import { Briefcase, Rocket, Settings, LineChart, Home as HomeIcon, Target, FileText, DollarSign, Building, CreditCard, PiggyBank, MapPin, Landmark, Users, ClipboardCheck, Banknote, ShieldCheck, ShoppingBag, BarChart2, Laptop, Siren, HeartHandshake, LandmarkIcon, Award, Trophy, Star, CheckCircle, ArrowLeft, ArrowRight, Link as LinkIcon, Search, MessageSquareQuote, Mail, Send, UploadCloud } from 'lucide-react';
...
const getTaskDetails = (taskId: string, taskActions: TaskActions): React.ReactNode => {
  switch (taskId) {
    ...
    case 'business-credit': return (<ul><li><CreditCard className="inline w-4 h-4 mr-1 text-accent"/>Learn how to build credit for your company.</li><li>Understand the basics with the <a href="https://www.sba.gov/business-guide/launch-your-business/establish-business-credit" target="_blank" rel="noopener noreferrer" className="underline hover:text-accent">SBA's guide <LinkIcon className="inline w-3 h-3"/></a>.</li></ul>);
    case 'fund-business': return (<div><p className="mb-2">Explore various options to finance your startup. Aim to:</p><ul className="list-disc pl-5 space-y-1"><li><strong>Research Grant Opportunities:</strong> Use the AI Grant Finder tool and platforms like <a href="https://www.grants.gov/" target="_blank" rel="noopener noreferrer" className="underline hover:text-accent">Grants.gov <LinkIcon className="inline w-3 h-3"/></a>. Identify at least 3 potential grants to investigate further.</li><li><strong>Explore Alternative Financial Instruments:</strong> Consider options like SBA loans (see <a href="https://www.sba.gov/funding-programs" target="_blank" rel="noopener noreferrer" className="underline hover:text-accent">SBA programs <LinkIcon className="inline w-3 h-3"/></a>), angel investors, venture capital, crowdfunding, or personal loans. Identify 2 alternatives that might fit your business.</li><li><strong>Draft Budget and Cash Flow Projections:</strong> Ensure you have a clear understanding of your startup costs (from previous task) and can project your income and expenses for at least the first year. This is crucial for any funding application.</li></ul><p className="mt-2">The AI Business Advisor can help create a Pitch Deck for investors or loan applications.</p></div>);
    case 'buy-existing': return (<ul><li><Building className="inline w-4 h-4 mr-1 text-accent"/>Weigh the pros and cons of buying vs. starting from scratch.</li><li>The <a href="https://www.sba.gov/business-guide/plan-your-business/buy-existing-business-or-franchise" target="_blank" rel="noopener noreferrer" className="underline hover:text-accent">SBA has resources <LinkIcon className="inline w-3 h-3"/></a> for this.</li></ul>);
    case 'pick-location': return (<ul><li><MapPin className="inline w-4 h-4 mr-1 text-accent"/>Choose the right physical or virtual space.</li><li>Guidance from the <a href="https://www.sba.gov/business-guide/launch-your-business/pick-your-business-location" target="_blank" rel="noopener noreferrer" className="underline hover:text-accent">SBA <LinkIcon className="inline w-3 h-3"/></a>.</li></ul>);
    ...
    case 'ai-small-business': return (<ul><li><Laptop className="inline w-4 h-4 mr-1 text-accent"/>Leverage artificial intelligence tools for efficiency.</li><li>Consider using the Text Analyzer in the AI Business Advisor below!</li></ul>);
...
export default function HomePage() {
  ...
  const handleCheckChange = (taskId: string) => {
    ...
    if (task && task.requiresUpload) {
        toast({
            variant: "default",
            title: "Upload Required",
            description: "Please upload the required document to complete this task.",
        });
        return; // Prevent checkbox toggling if upload is required
    }
  ...
  const handleFileUpload = async (file: File, taskId: string) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Upload Failed",
        description: "There was an error uploading your document. Please try again. (Ensure you are logged in if storage rules require auth)",
      });
    } finally {
